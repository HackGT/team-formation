    ...
    return (
        <div>
            <Form className="form-container">
                <Form.Group>
                    <Form.Input className="input-container-large" label='Full Name' placeholder='Full Name' defaultValue={props.name} onChange={e => updateName(e.target.value)} error={nameProfane} required="required"/>
                </Form.Group>
                <Form.Group className="school-and-year">
                    <Form.Select className="input-container-small" label='School' placeholder='School' defaultValue={props.school} onChange={(e, {value}) => updateSchool(value?.toString())} error={schoolProfane}
                    options={schools}
                    required="required"/>
                    <Form.Select className="input-container-small" required="required" label='Year in School' defaultValue={props.grad_year} onChange={() => setGradYear} options={years} placeholder='Year in School'/>
                </Form.Group>
                <Form.Group>
                    <Form.Input className="input-container-large" label='Track' defaultValue={props.track} onChange={() => setTrack}>
                        <input defaultValue={props.track} disabled/>
                    </Form.Input>
                    
                </Form.Group>
                <Form.Group>
                    <Form.Input className="input-container-large" label='Location' defaultValue={props.track} onChange={() => setLocation}>
                        <input defaultValue={props.location} disabled/>
                    </Form.Input>
                </Form.Group>
                <Form.Group>
                    <Form.Select className="input-container-large" label="Skills" placeholder='Skills' defaultValue={props.skills} onChange={() => setSkills} fluid multiple selection="selection" search options={allSkills}/>
                </Form.Group>
                <Form.Group>
                    <Form.TextArea className="input-container-large" label='Bio' placeholder='Introduce yourself!' defaultValue={props.experience} onChange={(e, {value}) => updateExp(value?.toString())} error={experienceProfane}/>
                </Form.Group>
                <Form.Group>
                    ...
                    onChange={e => updateContact(e.target.value)} error={contactProfane} required="required"/>
                </Form.Group>
                <Form.Group>
                    <div className="editCheckbox">
                        <Checkbox label='Make my profile public' onChange={() => setVisible} checked={(props.visible === 1)}/>
                    </div>
                </Form.Group>
